...
from model.imbalanced_sampler import ImbalancedDatasetSampler
...
class ABAW5Dataset(data.Dataset):
    def __init__(self, split='Train', transforms=None):
        ...
        if split != 'Test':
            data_dict = \
                np.load(pathlib.Path(self.root_dir, '{}.npy'.format(self.task)).__str__(), allow_pickle=True).item()[
                    self.split]
            renet18_feature = pickle.load(open((pathlib.Path(self.root_dir) / 'affectnet-resnet50-VA.pkl').__str__(), 'rb'))[self.split]
        else:
            data_dict = pickle.load(open((pathlib.Path(self.root_dir) / 'affectnet-resnet50-test-VA.pkl').__str__(), 'rb'))
        ...
        for vid in data_dict:
            ...
            cur_feature_df = data_dict[vid]['features']
            # num_frames = cur_vid_df.shape[0]
            num_frames = cur_feature_df.shape[0]
            ...
            for idx in range(num_seqs):
                if self.sampling_method == 'sequentially' or self.seq_len == 1:
                    ...
                    # Get the sequence
                    cur_seq = cur_vid_df['keep_frames'][st_idx: ed_idx, :]
                    cur_seq_feature = cur_feature_df[st_idx: ed_idx, :]
                ...
                if cur_seq.shape[0] < self.seq_len:
                    ...
                    cur_seq_feature = np.pad(cur_seq_feature, ((0, self.seq_len - cur_seq_feature.shape[0]), (0, 0)), 'edge')

                self.data_seqs.append(cur_seq)
                self.feature_seqs.append(cur_seq_feature)
    ...
    def __getitem__(self, index):
        ...
        cur_feature_sample = self.feature_seqs[index]

        cur_seq_index = np.array([int(x.split('/')[-1][:-4]) for x in cur_seq_sample[:, 0]])

        sample = {'image': [], 'index': cur_seq_index, 'feature': torch.from_numpy(cur_feature_sample),
        ...
        for idx in range(cur_feature_sample.shape[0]):
            # Load image
...
class ABAW5DataModule(LightningDataModule):
    def __init__(self):
        super(ABAW5DataModule, self).__init__()
        ...
        # aug = [transforms.Resize((224, 224)), transforms.RandomHorizontalFlip(p=0.5),
    ...
    def setup(self, stage=None):
        if stage == 'fit' or stage is None:
            ...
            self.train_dataset = ABAW5Dataset(split='Train', transforms=self.transforms_train)
            self.val_dataset = ABAW5Dataset(split='Validation', transforms=self.transforms_test)

        if stage == 'test' or stage is None:
            # Create test set
            self.test_dataset = ABAW5Dataset(split='Test', transforms=self.transforms_test)
            # self.val_dataset = ABAW5Dataset(split='Validation', transforms=self.transforms_test)
